...
if (!config.isUaaConfigured()) {
  ...
  if (config.mytestDataURL && config.mytestDataURL.indexOf('https') === 0) {
    app.get('/api/testdata/*',
        proxy.addClientTokenMiddleware,
        proxy.customProxyMiddleware('/api/testdata', config.mytestDataURL, '/services/experience/datasource/testdata'));
  }
...
var mockMytestDataRoutes = require('./routes/mock-mytest-data.js')();
...
app.use(['/mock-api/testdata', '/api/testdata'], jsonServer.router(mockMytestDataRoutes));
